...
export async function POST(req: NextRequest) {
    ...
    // Row 0 is header, row 1 is test entry (skip both)
    const dataRows = rows.slice(2).filter(r => r[1]?.trim())
    ...
    await Db.deniedApplicationsHQ.deleteMany({})
    if (docs.length > 0) await Db.deniedApplicationsHQ.insertMany(docs as any)
