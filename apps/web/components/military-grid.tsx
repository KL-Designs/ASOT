 ...
 */
export default function MilitaryGrid({ gradient, style }: {
    gradient?: boolean
    style?: React.CSSProperties
}) {
    const mask = gradient ? 'radial-gradient(ellipse at 50% 50%, black 50%, transparent 80%)' : ''
    return (
        <div aria-hidden className='absolute inset-0 pointer-events-none select-none' style={{ zIndex: 0, maskImage: mask, WebkitMaskImage: mask, ...style }}>
