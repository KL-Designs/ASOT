...
Route `/dashboard/j4`. Gated by `PERMISSIONS.departments.j4`; computes `canManageLinks` as `hasPermission(me, 'departmentLeads.j4')` OR `hasDepartmentPermission(me, 'j4', 'deptLinks.manage')`, plus `canBackupManage` (`hasPermission(me, 'backups.manage')`) and `canBackupRestore` (`hasPermission(me, 'backups.restore')`). Renders `J4AdminPanel` (no separate `canManageMembers`/`isJ4` props needed; J4 members are always leads/J4).

#### app/dashboard/j4/J4AdminPanel.tsx
Main J4 panel. Tabs: Mastersheet (0, `MasterSheetTab`), Tickets (1, `CommunityTicketsTab`), Meetings (2, `J4MeetingsTab`), Backups (3, `BackupsTab` — rendered only when `canBackupManage`, and passed `canRestore={canBackupRestore}`), Teamspeak (4, `TeamspeakTab`), Tools (5, grid of action tiles, not a separate component file), AI Admin (6). Every `<Tab>` carries an explicit `value={n}`: the Backups tab is conditional, and without pinned values MUI would renumber the tabs after it by position while the panel bodies still test the original indices. A requested tab of 3 falls back to 0 when `canBackupManage` is false, since the index survives in bookmarks and pinned sidebar links. Header toggles: Settings (`DeptSettingsView department='j4'`, `canManage={true}` kept exactly as before, plus the new `canManageLinks` prop), Calendar (`DeptCalendarTab department='j4'`), Activity Logs (`LogsTab`, note: uses `view==='logs'` not `'activity'`); `DeptLinksRail` renders above the Tabs strip on the `dept` view.
Contains four modals defined inline: `CpuProfileModal` (lists captured V8 CPU profiles via `GET /api/admin/diagnostics/cpu-profile`, starts a timed capture via `POST /api/admin/diagnostics/cpu-profile?duration=<secs>`, and links each result to `GET /api/admin/diagnostics/cpu-profile/<filename>` for download), `DischargeModal` (submits `j4-discharge` ticket via `POST /api/admin/tickets`, requires target member/discharge-type/reason), `ReinstateModal` (two-step: pick discharged member from `GET /api/admin/members/discharged`, then choose which snapshot data — qualifications/awards/trainings/campaign medals — to restore via `PATCH /api/admin/members/discharged`), `TestNotificationModal` (send a test notification to self or another member via any of `NOTIF_TYPES`, channels site/discord, via `POST /api/admin/notifications/test`).
Tools grid also includes: Import Panel (opens shared `ImportPanel` from `app/dashboard/j4/../ImportPanel` i.e. `app/dashboard/ImportPanel.tsx`), Discord Developer Mode toggle (`GET/POST /api/admin/discord-devmode`), TeamSpeak Developer Mode toggle (`GET/POST /api/admin/teamspeak-devmode`), Test Notification launcher, link to HQ Mastersheet (tab 0), link to `/dashboard/j4/website-settings` (Website Settings).

#### app/dashboard/j4/website-settings/page.tsx
Route `/dashboard/j4/website-settings` ("Website Settings"). Client component, no explicit auth check in this file itself (relies on being linked only from the gated J4 panel, and on every API it calls gating on `PERMISSIONS.departments.j4` — **note**: this page does not re-verify server-side; it's a client page with no page-level guard). Five sub-tabs:
...
- `RecruitmentSettingsPanel` — public recruitment flow: info-page visibility toggle and the info-page content itself. `GET/PUT /api/admin/recruitment-settings`, `/api/admin/recruitment-info`, `POST/DELETE /api/admin/recruitment-images`.
- `CertificateSignatoryPanel` — picks which India Company HQ ORBAT **position** signs rendered certificates (a position, not a person, so a change of command needs no edit here); shows each CHQ slot with its current holder, marks the auto-picked slot when unset, and re-selecting the chosen slot clears back to auto. Only the fallback — an award or promotion recording its own issuing officer is signed by them. `GET/PUT /api/admin/certificate-signatory`.

#### app/dashboard/j4/BackupsTab.tsx
Restic-backed backup manager. Takes a required `canRestore` prop (`backups.restore`): the per-row Revert button and the whole Upload &amp; Revert block are hidden without it, while the timeline, create-now, download, storage breakdown and config panel need only `backups.manage` (which the parent already required to render this tab at all). Merged DB/media timeline (create-now/revert/upload/download, no manual delete — retention is automatic via `restic forget`), **collapsed to the latest 10 rows** with a "Show all N backups" toggle — hourly retention runs the list into the hundreds and every row carries its own hover state and up to four buttons, so rendering them all costs something on each re-render. The collapsed control names how many manual and pre-restore points are hidden, since those are the ones a person deliberately made and burying them unlabelled is the one way this could mislead; rows whose snapshot carries the `pre-restore` tag show a "Pre-restore" badge beside the timestamp, marking the safety copy taken automatically before a restore. Also: a restic-binary-health chip, a live-vs-backed-up storage usage breakdown (two donut charts), in-progress operation status with a size-aware estimated duration (via localStorage history of `{durationSecs, sizeBytes}` samples, deriving a rate rather than a flat average once size data exists), and tiered-retention settings (`keepHourly`/`keepDaily`/`keepWeekly`/`keepMonthly`) whose steppers floor at the currently saved value — the API rejects any reduction, so retention is extendable from the UI but never reducible.
**Upload & Revert shows real transfer progress** — percentage, bytes, rate and ETA — via `XMLHttpRequest` rather than `fetch`, which is not a style choice: fetch cannot report upload progress at all, since its request body is not observable, and `xhr.upload.onprogress` is the only browser API that can. The rate is an exponential moving average; the raw per-event figure swings too hard for a readable ETA. Two phases are shown as two things: a determinate bar while bytes move, then indeterminate with "Uploaded — restoring on the server" once they have all arrived, because the server restores before it responds and a determinate bar pinned at 100% for minutes reads as a hang.
