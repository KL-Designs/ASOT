...
| Retired members (CSV import + public memorial wall) | A (`/api/admin/retired/import`), D (`/api/community/retired[/snapshot]`), F (`dashboard/retired/page.tsx` — **no page-level permission gate**, flagged), G (`retired/RetiredWall.tsx`), H (`types/retired-member.d.ts`) |
...
| ORBAT (structure, positions, reservists, section meta, move requests) | A (`/api/admin/orbat/**`), F (`dashboard/orbat/OrbatManager.tsx`), G (`orbat/page.tsx` public board), H (`lib/orbat/**`, `types/orbat.d.ts`) |
...
| Quiz (BCT confirmation quiz, assign/review) | A (`/api/admin/quiz/**`), D (`/api/community/quiz/[attemptId]`), F (`dashboard/quiz/review/**`), G (`quiz/[attemptId]/**`), H (`lib/quiz-data.ts`, `types/quiz.d.ts`) |
...
| Kits / loadouts (ACE arsenal import, per-member kit tab, the unit's shared-kit shelf, star ratings, kit tags/filtering, copy counts) | G (`milpacs/[username]/loadout-panel.tsx`, `loadout-manager.tsx`, `kits/**`), D (`/api/loadouts/**`, incl. `[id]/rating`, `[id]/copy`), H (`lib/loadout/**` incl. `rating.ts`/`tags.ts`/`shelf.ts`, `components/loadout/stars.tsx`/`tag-chips.tsx`, `types/loadout.d.ts`) — "kit" is the reader-facing word, "loadout" the code's |
| HZN-MED medical minigame (`/ace` — ACE/KAT parody: wounds, dressings, airway, drugs, rhythms, overdoses) | G (`ace/**`, and `milpacs/[username]/medical-menu-link.tsx` — the only link to it, on one member's milpac); unlisted rather than gated, no API routes, nothing persisted |
| Gallery / Screenshot of the Month / the featured rail (`featuredOrder`: membership from `PATCH /api/gallery/admin/media/[id]`, arrangement from `PUT /api/gallery/admin/featured/order`, cap in `lib/gallery/featured.ts`) | A (`gallery/admin/*` in D's scope note — actually under D as non-admin fetch/featured/sotm; admin folder mgmt lives in E `J5Panel` tabs calling `/api/gallery/admin/**`), D (`/api/gallery/**`), E (`j5/tabs/GalleryOperationsTab.tsx` etc.) |
| Gallery storage grammar (which folder a photo's bytes are filed into, the `year`/`campaign`/`operation`/`mission` facets, the Media tab's rail tree, reconciling disk against database) | H (`lib/gallery/**` — `content-path.ts`, `relocate.ts`, `operation-facets.ts`, `reconcile.ts`, `naming.ts`, `paths.ts`, `deps.ts`, `library-query.ts`; `types/gallery.d.ts`, `types/gallery-media.d.ts`), F (`j5/tabs/media/**`) |
| J5 console controls (the department's own field/select/tag-picker components and their styles — not MUI) | F (`j5/controls/**`), plus `styles/j5-fields.module.css` and `styles/j5-controls.module.css` |
