...
| Kits / loadouts (ACE arsenal import, per-member kit tab, the unit's shared-kit shelf, star ratings, kit tags/filtering, copy counts) | G (`milpacs/[username]/loadout-panel.tsx`, `loadout-manager.tsx`, `community/kits/**`), D (`/api/loadouts/**`, incl. `[id]/rating`, `[id]/copy`), H (`lib/loadout/**` incl. `rating.ts`/`tags.ts`/`shelf.ts`, `components/loadout/stars.tsx`/`tag-chips.tsx`, `types/loadout.d.ts`) — "kit" is the reader-facing word, "loadout" the code's |
