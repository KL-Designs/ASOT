...
- **GET** — a downscaled WebP resized once from the original (or, for a video/embed, from its existing `posterKey` still — no ffmpeg on this path) and cached on disk under `storage/gallery/thumbs/{id}-{width}.webp`. Exists because the J5 Media tab was rendering full-size originals in 178px tiles: 4,781 archive files averaging 3.8MB, sixty a page, ~200MB per page view. **`?w=` picks one of three widths and is an allow-list, never a free integer** — 400 (default, J5 grid tile), 800 (public featured rail, home-page mosaic single-column tiles), 1600 (home-page mosaic double-width tiles). Every distinct width is a file written into the bind-mounted thumb cache and never evicted, so a free `?w=` would be a resize-on-demand endpoint an anonymous visitor could loop over to fill the volume; `parseThumbWidth` folds anything unrecognised back to 400 rather than erroring, because there is no caller to correct and a broken image would be the only effect. Size/format/path logic is in `lib/gallery/thumbs.ts` (`ThumbWidth`, `THUMB_WIDTHS`, `THUMB_WIDTH`, `FEATURED_THUMB_WIDTH`, `FEATURED_WIDE_THUMB_WIDTH`, `THUMB_QUALITY`, `parseThumbWidth`, `thumbPath`, `thumbUrl` — the id is re-validated against `/^[0-9a-f]{24}$/` and the resolved path asserted inside `THUMB_DIR`). Cache entries are invalidated against the **source's** mtime, and the weak ETag is over the source's size+mtime plus the RESOLVED width (which is what stops a client holding a 400px thumbnail being told its 800px request is unchanged — with three live widths on one URL, nothing else in the tag differs) — no `immutable`, for the reason `/api/gallery/media/[id]` documents. Generation writes to a temp name and `rename()`s into place so concurrent requests never read a half-written file; any resize failure logs and 307-redirects to the full-size media/poster route, so a broken pipeline degrades to slow rather than to blank tiles. Auth: same gate as `/api/gallery/media/[id]` — public for `live`, author or `gallery.review` for `pending`/`processing`, 404 (never 403) otherwise. Collections: `Db.galleryMedia`. Consumed by `MediaGrid.tsx` and `MediaTable.tsx` via `AdminMediaAPI.thumb` from `/api/gallery/admin/library` (400); by `FeaturedRail.tsx` and the J5 Featured tab's rotation tiles via `FeaturedItemAPI.thumb` from `/api/gallery` (800); and by `GalleryStrip.tsx` via `GalleryTile.thumb`/`thumbWide` from `lib/landing.ts` (800/1600). The gallery **lightbox** deliberately keeps `src`, the full original — someone who opens a photograph is asking to look at it.
...
### gallery/admin (4 files catalogued here)

The rest of `app/api/gallery/admin/**` — the J5 media console's facets/tree/bulk/health endpoints — is not yet catalogued in any part file; this section holds the ones that are.

#### /api/gallery/admin/media/[id]
- **PATCH** — edits one archive item: `caption`, `tags`, the author PAIR (`authorId` + `authorName`, resolved through `lib/gallery/author.ts` so the two can never name different people), `mission`, `operationId`, and `featured`. **Reassigning the operation moves the file on disk** via `relocateMedia`, and whether that happens is derived from the REQUEST, never from the document: an unlinked archive item (folder-derived `operation`/`opLabel`/`year`/`mission`, no `operationId`) relocates only when the payload actually asks to change its operation, because relocateMedia would otherwise resolve `Unknown/` and erase all four. **`featured` is deliberately outside that condition** — it is rail membership, not filing. Featuring appends (`featuredOrder` = current highest + 1, re-stating the existing slot rather than re-appending when the item is already in the rail, so a curated position survives a double click); unfeaturing `$unset`s the field, because the rail's query is `featuredOrder: { $exists: true }` and any sentinel number would still be in it. Refuses at `MAX_FEATURED` (`lib/gallery/featured.ts`), the same cap the order route enforces. Auth: `hasPermission(me, 'gallery.manage')`. Collections: `Db.galleryMedia`, `Db.galleryTags`, `Db.users`, `Db.operations`.
- **DELETE** — the only removal path for a media record: deletes the document first, then its bytes and poster, then its votes. Record-then-bytes on purpose — a failed unlink leaves an orphan the Health view reports, while the reverse would leave a permanently broken tile. Auth: same. Collections: `Db.galleryMedia`, `Db.galleryVotes`.
- Tests: `route.test.ts` beside it (operation reassignment on bytes-less/bytes-missing items, the author pair, the featured rail).

#### /api/gallery/admin/library
- **GET** — the Media tab's centre pane: one page of `AdminMediaAPI` rows plus `total`, filtered/sorted/paged by `lib/gallery/library-query.ts`. Separate from the facets route because this refetches on every keystroke while the facets aggregate the whole collection. Carries what the public route deliberately does not — `storageKey`, `authorId` — plus `thumb` (the ~400px cached WebP, not the original) and `featured` (whether `featuredOrder` is set; the number itself is not sent, because arrangement belongs to the Featured tab). Auth: `hasPermission(me, 'gallery.manage')`. Collections: `Db.galleryMedia`.

#### /api/gallery/admin/featured/order
- **PUT** — sets the public featured rail's ORDER from a complete, front-to-back `ids` array; anything live and currently ordered but absent from it drops out. De-duplicates before applying the `MAX_FEATURED` cap (`lib/gallery/featured.ts`) and silently ignores ids that are no longer live. Writes the new `featuredOrder`s **first** and unsets the departed **second**, deliberately: a crash between the two shows a visitor a duplicated tile rather than an empty strip. It owns arrangement; `PATCH /api/gallery/admin/media/[id]` owns membership, and both refuse at the same cap. Auth: `hasPermission(me, 'gallery.manage')`. Collections: `Db.galleryMedia`.
