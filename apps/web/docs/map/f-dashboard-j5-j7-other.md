...
J4-only (`manageOrbatRoles`) wide `Dialog` for CRUD on the `OrbatRole` catalog: list/create/edit/delete, each Role's name, optional `tag`, category scope (`PLATOON_CATEGORIES` multi-select, empty = all), granted Discord roles (multi-select fetched from `/api/admin/orbat/discord-roles`), and granted permission keys (multi-select fetched from `/api/admin/orbat/permission-keys`). The Categories/Discord-roles/Permissions sections lay out as a `flex-wrap` row of columns on wide screens instead of a fixed-width vertical stack. Each section has copy/paste icon buttons pulling from/pushing to an in-memory clipboard (cleared on dialog close); "Copy Settings"/"Paste Settings" buttons operate on all three clipboard slots at once, letting an admin template one role's categories/Discord-roles/permissions onto another without touching Name/Tag. The editor tracks unsaved changes (`dirty`) and blocks switching roles, starting a new role, or closing the dialog (button, Escape, backdrop) via `window.confirm()` until the pending edit is saved or explicitly discarded; Save is pinned in a non-scrolling footer alongside Discard. Delete shows the API's `inUseCount` error inline rather than silently failing. Calls: `GET/POST /api/admin/orbat/roles`, `PATCH/DELETE /api/admin/orbat/roles/{roleId}`, `GET /api/admin/orbat/discord-roles`, `GET /api/admin/orbat/permission-keys`.
