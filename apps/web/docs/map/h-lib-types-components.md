...
- `hasPermission(user, key): Promise<boolean>` — additive permission check: true if the user's Discord roles satisfy `PERMISSION_CATALOG[key]` (existing mechanism, untouched) **or** the user's currently-assigned `Db.orbatPositions` doc's `roleId` resolves to an `OrbatRole` whose `permissions` includes `key`. Only ever widens access relative to the existing check. Not yet wired into any route as of this writing (mechanism-only — see `docs/superpowers/specs/2026-07-13-orbat-roles-design.md` for the deliberate scope boundary) — routes still call `client.hasRoles()` directly.
...
- `categoriesOverlap(a, b): boolean` — two category-scope arrays overlap if either is empty (unscoped = matches everything) or they share an element. Used to allow two `OrbatRole` catalog entries to share a `name` as long as their `categories` don't overlap (e.g. two "Section Commander" Roles scoped to different platoons).
