...
- **`naming.ts`** — `splitOperation`, `operationDisplayName`, `normalizeKey`, `fullKey`/`strippedKey`, `findByOperationKey`. `operationDisplayName(opLabel, operation)` is what every surface that can only fall back to a RAW folder name calls (the J5 media table/inspector/viewer, the public facet rail) — it returns `opLabel` untouched and strips the order prefix off the folder fallback, so nothing user-facing ever prints "15. Op Black Hills". The **two-tier** folder↔operation match (parenthetical-carrying key first, then stripped). The ORDER is the safety property — read the module comment before touching it.
