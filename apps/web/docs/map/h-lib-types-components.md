...
- `hasPermission(user, key): Promise<boolean>` — additive permission check: true if the user's Discord ID is in the `OVERRIDE` env list (the only hard bypass) **or** the user's currently-assigned `Db.orbatPositions` doc's `roleId` resolves to an `OrbatRole` whose `permissions` includes `key` **or** their base department role (implicit from `User.departments`) or any assigned department sub-role (from `User.departmentRoleIds`) grants it via `Db.departmentRoles` **or** the permanent **Members** role (`department: 'members'`, `isBase: true`) grants it and they hold any `Db.orbatPositions` doc at all. The positions query is deliberately NOT filtered on `roleId` — it answers both "which ORBAT Roles do they hold" and "are they in the ORBAT", and a callsign position whose `roleId` was never set is still the latter; reservists need no special case, their positions live in the same collection. The Members arm costs no extra query: it reuses that positions result and adds one clause to the existing `departmentRoles` `$or`. Deliberately does NOT fall back to checking raw Discord role names — this function replaces that pattern, one permission key at a time, across the site. Wired into `attendance.confirm` (pre-existing) and, as of the permission-system migration, `pages.member` (35 call sites) — reservists can now satisfy this via their seeded "Reservist" `OrbatRole` (see `lib/orbat/reservist-role.ts`).

### lib/orbat/hasPermissions.ts
- `hasPermissions(user, keys: string[]): Promise<Record<string, boolean>>`: batch variant of `hasPermission`, answers every requested key in one query pass (one `orbatPositions`/`orbatRoles` round trip, one `departmentRoles` round trip) instead of repeating 2-3 uncached queries per key. Identical semantics to `hasPermission`: same `OVERRIDE`-only hard bypass, same four grant sources (ORBAT position Role, the permanent Members role for anyone holding an ORBAT position, base department role, department sub-roles).
...
### lib/operations/aar.ts
- `aarOpen(stage)` — the AAR opens at `confirmations_open`, which *is* "the operation has finished": it is the stage the cron moves to when the night ends and the one that sets the status to Completed. Using it avoids a second notion of finished that could disagree with the first. Stays open at `completed` — an AAR that closed with the paperwork would lose everyone who writes theirs up the next morning.
- **`sectionLead(roster, sectionTitle)`** — the 1IC: the occupant of the section's lowest-`order` **filled** slot. **Deliberately not the ORBAT's `isSenior`**, which is who leads on paper. The two disagree on exactly the nights this matters — a stand-in commander is an ordinary Saturday, and asking the absent paper leader to write up a section they did not lead is the wrong question. **This is the one piece of authority in the operations area that is positional rather than granted**, which is why it is a query and not a permission check.
- `sectionsOf` / `ledSections` / `canWriteSection(roster, userId, section, canManageAll)` — the sections on the night, the ones a member led, and the two ways in: positional (led it) or granted (`aar.manage`, so somebody can close an operation out when a 1IC never does).
- `didAttend(record, heldAPosition)` — gates feedback. Confirmation is definitive but arrives late, so an unconfirmed member who said yes *and* held a position counts until somebody says otherwise; an explicit non-attending status always wins. Without that, nobody could leave feedback until their 1IC did paperwork.
- `ATTENDANCE_STATUSES` — the eight (ATTENDED / NOT ATTENDING / RESOLVED / NO NOTICE / LOA / CONFIRM / N/A / ADDED TO UNIT), **moved here** from a private constant in `components/operations/AttendancePanel.tsx`. Two copies of "what states can a member be in" is how the board and the write-up end up disagreeing about a night that already happened. Each carries `present`, which is what drives confirmation and the billet rollup.
- `RATING_SCALE` / `RATING_ASPECTS` — **why it is not stars.** The question is "how did this compare to a normal night", which makes the middle *neutral*, not mediocre. Stars cannot say that: 5 reads as the goal and 3 as a disappointment, so a run of perfectly good ordinary operations would score 3/5 and look like a problem. Stored 1–5, drawn as a diverging bar centred on "As usual" with `offset` from -2 to +2.
- `validRating` / `averageRating` — the latter rounds to one decimal, because an average of "compared to usual" carries about that much signal and quoting 3.47 implies otherwise.
- `aar.test.ts` — 42 tests, including every fallback case for `sectionLead` and every branch of `didAttend`.
...
### lib/permissions/tree.ts
- `buildPermissionsTree(): Promise<PermissionCategory[]>` and `buildMemberGrants(userId)` — back the Roles Manager’s Permissions Explorer (`GET /api/admin/permissions/tree`, `GET /api/admin/permissions/member/[id]`): every permission key with the Discord/ORBAT/department roles that grant it and a live holder count, and one member’s full granted/denied breakdown with the reason for each.
- **A second implementation of `hasPermission`’s rules**, because it has to answer for every member at once and say *why*. Any arm present in one and missing from the other makes the Explorer lie — which matters most here, since the Explorer is what an admin opens to ask why somebody can do something. `resolveGrant()` therefore mirrors `hasPermission` arm for arm, including the Members role: reached from `orbatMemberIds` (anyone holding a `Db.orbatPositions` doc, roleId or not), never from `User.departments`, which can never contain `members`.
...
- `MEMBERS_DEPT` (`'members'`), `PSEUDO_DEPT_CODES`, `PSEUDO_DEPT_LABELS` — pseudo-departments: entries that live in the `department_roles` catalog and are edited through the same Departments tab, but are not real departments. Deliberately NOT part of `DEPT_CODES`, which drives department-membership sync, `DeptMembersTab`, ticket routing and leadership slots — folding them in would create a phantom eighth department in all of those. `members` holds exactly one permanent base role, takes no sub-roles, and nobody is ever assigned it: `hasPermission`/`hasPermissions`/`lib/permissions/tree.ts` resolve it from the member’s ORBAT position, and `lib/orbat/grant-bundles.ts` owes its Discord/TS grants on the same terms.
...
### lib/orbat/grant-bundles.ts
- `grantBundlesFor(user, catalog: GrantCatalog): GrantBundle[]` — the expected-grant rules for one member: which Discord roles and TeamSpeak groups they are owed, and what owes each. Pure and dependency-free (takes already-loaded catalogs, never touches `Db`), extracted from `member-sync.ts` so the rules are unit-testable without booting Discord and TeamSpeak first. Arms: each department base role they belong to; each held sub-role scoped to a department they are actually in; **the Members role, for anyone holding any ORBAT position** (source label `Members: everyone in the ORBAT`); their ORBAT position's Role; and the category/section metadata grants. A grant appearing in more than one bundle is normal — `mergeBundles` upstream keeps every contributing source.
- Interfaces: `GrantBundle`, `GrantCatalog`.
...
- `revertToPoint(point, parts?, { wipeMedia?, wipeDatabase? })` — takes a safety backup first via `runSafetyBackup()` (aborts, restoring nothing, if that fails), then restic-restores whichever side(s) are present to temp dirs, drops+recreates every DB collection from EJSON (recreating the two critical `orbat_positions` indexes), copies restored media over `gallery`/`uploads`. `wipeDatabase` additionally drops every collection the *live* database reports (`listCollections()`, `system.*` excluded) before reinserting, so a collection created after the backup was taken doesn't survive a restore to before it existed — the bug that left `gallery_media` behind and had the J5 rail drawing folders that were only in the database. Both wipe flags default to `false` at the library level (a caller that says nothing gets the non-destructive behaviour) and both are ticked by default in the dashboard. The DB wipe sits *inside* `collectionNames.length > 0`, mirroring `wipeMedia`'s placement inside `if (gallery)`: an empty, missing or unreadable dump can never empty the database and leave nothing to put back.
...
- `openDownloadZipStream(point, parts?, { galleryNumbering? })` — restores a point to a temp tree and returns a web `ReadableStream` of a zip built from it on the fly. Replaced `buildDownloadZip()`, which restored (7GB on the current media set), copied into a staging tree (another 7GB), then zipped that to disk (another ~7GB) — ~21GB of temp space, none of it reaching the caller until it was all done. Peak is now the restore alone. The stream owns the temp tree and deletes it via `stream.finished()` on end, error, or client abort — never sooner, since `archiver` reads lazily as the response drains. `assertRoomForRestore()` prechecks free space (`statfs(tmpdir())` vs restic's own `total_bytes_processed`, skipped when either is unavailable) so a shortfall is an upfront error rather than a half-sent download. `galleryNumbering` (built by the download route from `lib/gallery/export-numbering.ts`, never here — this module must stay importable with no Mongo connection) re-adds the `{n}. ` order prefix to operation folders inside `gallery/content`; it reaches the entries through `zipEntryNameFor(prefix, header, rename?)`, so the compatibility contract still covers the exact name that lands in the zip.
...
- `applyUploadedZip(zipPath, parts?, { wipeMedia?, wipeDatabase? })` — same options and same defaults as `revertToPoint`. Takes a safety backup first via `runSafetyBackup()` (aborts, extracting nothing, if that fails), then validated entry-by-entry extraction (rejects zip-slip paths and symlink entries — the source is an untrusted upload) of the same `{db-source/, gallery/, uploads/}` shape, then the same restore logic as `revertToPoint`. Does not feed the upload into either restic repo's history.
...
### lib/gallery/ — the content-tree path grammar

The gallery is Mongo-backed (`Db.galleryMedia`), but the bytes still live in a readable folder tree under `storage/gallery/content` and **the folders and the document must never disagree** — a human opening a downloaded backup reads the folders, and the Media tab's rail filters on the fields. These five modules are that contract.

- **`featured.ts`** — one constant, `MAX_FEATURED` (60): how many items the public featured rail holds. Two routes write that rail — `PUT /api/gallery/admin/featured/order` (arrangement, whole list) and `PATCH /api/gallery/admin/media/[id]` (membership, one item, appends) — and a cap only one of them enforced would let a reviewer toggle in a sixty-first item and discover it much later, as a refused drag. It lives in `lib/` rather than in either route because a route file under Next's `typedRoutes` may export only route handlers and the permitted config names; a stray `export const` there breaks the production build while tsc, ESLint and vitest all stay green.

- **`content-path.ts`** — pure (no fs, no mongodb, no imports). `MAX_SEGMENT` (120), `UNKNOWN_FOLDER`, `sanitizeSegment(raw)` (strips separators, the Windows-reserved set and control characters; the trailing-dot trim is also what neutralises `.`/`..`), `parseContentPath(relative)` → `{year, campaign, operation, mission, file} | null`, `buildContentPath(f)`. The grammar, two to five segments:
  - `{year}/{campaign}/{campaign mission}/{Saturday|Sunday}/{file}` — an operation in one of J2's campaigns
  - `{year}/{campaign}/{Saturday|Sunday}/{file}` — an operation whose campaign is known but whose `CampaignMission` record does not exist yet (J2's board infers those from titles until someone presses Auto-group). Structurally the single-mission shape with the campaign's name on the top folder: the `campaign` facet stays **null** on both sides, which is what keeps build and parse agreeing.
  - `{year}/{operation}/{Saturday|Sunday}/{file}` — a single mission with a day slot
  - `{year}/{operation}/{mission}/{file}` — legacy archive files
  - `{year}/{operation}/{file}` — a published submission with no day
  - `Unknown/{file}` — no operation, or none resolvable
  A **three-directory path is ambiguous** (legacy `op/mission`, single-mission `op/day`, campaign-with-no-day-slot `campaign/mission` and campaign-with-no-mission `campaign/day` all produce it) and is read the legacy way; see the comment on that branch for why that is the reading that cannot lose anything. `buildContentPath`'s Unknown guard is also what stops a campaign ever being emitted without an operation beside it.
- **`naming.ts`** — `splitOperation`, `operationDisplayName`, `normalizeKey`, `fullKey`/`strippedKey`, `findByOperationKey`. `operationDisplayName(opLabel, operation)` is what every surface that can only fall back to a RAW folder name calls (the J5 media table/inspector/viewer, the public facet rail) — it returns `opLabel` untouched and strips the order prefix off the folder fallback, so nothing user-facing ever prints "15. Op Black Hills". The **two-tier** folder↔operation match (parenthetical-carrying key first, then stripped). The ORDER is the safety property — read the module comment before touching it. `ORDER_PREFIX` **requires a separator** (`.`/`)`/`-`/en dash, or whitespace): with it optional, every leading digit run was a prefix, so `1st Recon Sweep` split to the label `st Recon Sweep` and `12` to order 12. That was a display bug until `scripts/strip-folder-numbers.ts` started renaming the folders it is told carry one. `order` now has exactly one consumer left — `export-numbering.ts`'s already-numbered guard — since every ordering surface moved to `takenAt`.
- **`relocate.ts`** — `operationYear(date)` (UTC, shared), `resolveOperationFolder(deps, operationId)` → `OperationFolder` (`{year, campaign, operation, mission}`), `relocateMedia(deps, id)`, and `RelocateDeps`. The **only** writer of `year`/`campaign`/`operation`/`opLabel`/`mission`/`takenAt` for an item that has bytes. The numbered `{n}. {name}` two-tier match applies to whatever occupies the top-level-within-year slot: the **campaign** for a campaign mission, the operation for a single mission — which is what stops three missions of one campaign minting three sibling folders. The campaign-mission folder is `CampaignMission.name` verbatim (no order prefix); the day folder comes from `Operation.daySlot` and is never guessed from a title's "— Sat"/"— Sun". An unusable **mission** beside a good campaign (no `campaignMissionId`, a deleted or missing mission, a mission filed under another campaign, a mission name that sanitizes away) drops the mission LEVEL and files under the campaign folder alone — with `campaign` null and the campaign's folder name in `operation`, because that is the folder directly above the day. An unusable **campaign** (`isSingleMission: true`, a `campaignId` naming nothing, a deleted campaign, a campaign name that sanitizes away) files as a single mission, never as an error or an `Unknown` folder. An operation with **no date** still goes to `Unknown/` regardless of its campaign. A folder minted here carries **no `{n}. ` order prefix** — the ordering lives in the database (`takenAt`) and is put back onto folder names only by a backup export (`lib/gallery/export-numbering.ts`); existing numbered folders are matched and reused unchanged, because `findByOperationKey` compares `splitOperation(folder).label`, which strips the prefix first. After a successful move it prunes the directory the file came from and every parent that is now empty, stopping at `CONTENT_DIR` and never following a symlink — a failure to prune never fails the move.
- **`operation-facets.ts`** — `operationFacets(deps, operationId)` → `OperationFacetUpdate`. The same resolution for anything with **no bytes** (an embed, a failed transcode), which `relocateMedia` returns early for. Must produce the identical shape, with one documented exception: it writes `mission` only when a day slot resolves and never unsets it, because it has no media document and so cannot tell a legacy mission folder from a stale day.
- **`reconcile.ts`** — `reconcile(deps)` → `ReconcileReport`. Four ordered rules (id beats path); **nothing here deletes a record or a file**. Walks the tree with an explicit depth cap that must move with the grammar — one level short does not error, it silently reports every campaign item as a missing file.
- **`paths.ts`** — `resolveStorageKey(key)`, the only thing between a `storageKey` and the filesystem; its own segment cap has to match `content-path.ts`'s.
- **`export-numbering.ts`** — `EXPORT_FOLDER_PIPELINE`, `foldersFromAggregate(raw)`, `buildFolderNumbering(rows)` -> `Map<'{year}/{folder}', '{n}. {folder}'>`, `numberContentEntry(name, isDirectory, numbering)`. Puts the order prefix back onto operation folders **inside a downloaded backup only** — a zip is browsed in a file manager with no database to sort it. Ranks a year's folders by earliest `takenAt` (undated last). Never renames a folder that already carries a prefix (the whole legacy archive does, and its documents are matched by PATH, so a rename would report every one of them as a missing file), never renames one holding a document that is not findable by the id in its filename, and never renames onto a name another folder in that year already has. Pure: no `@/lib/mongo`, so `lib/backups.ts` can import it.
- **`strip-folder-numbers.ts`** — `planFolderStrip(containers, keys)` -> `{renames, skips}`, `MAX_ORDER_DIGITS` (3). Pure (no fs, no mongodb): the PLAN for `apps/web/scripts/strip-folder-numbers.ts` (start menu → Migrations → *Strip: gallery folder numbers*), which takes the `{n}. ` prefix off the operation folders that still carry one and rewrites the `storageKey` plus the naming facet (`campaign` on a five-segment key, `operation` otherwise — the same split `parseContentPath` makes) of every document under them. Only the top-level-within-year slot is ever a candidate; campaign, mission and day folders are untouched. **Refuses and reports** a strip that would land on an existing folder, two folders that would strip to the same name, a destination path another document already claims (compared without the `content:`/`legacy:` prefix), a leading number too long to be an order, and a folder that is on disk under neither name. Candidates come from the storage KEYS as well as from the disk, which is what makes re-running it finish a folder that was renamed before its documents were updated. `idNamed` flags a folder holding path-matched (legacy) documents — still renamed, but `export-numbering.ts` can never number it inside a backup zip again.
- **`featured-order.ts`** — `planFeaturedOrder(featured, archive)` -> `{placements}`, plus `FeaturedCandidate`/`ArchiveCandidate`/`FeaturedPlacement`/`SelfReason`. Pure (no fs, no mongodb): the PLAN for `apps/web/scripts/backfill-featured-order.ts` (start menu → Migrations → *Backfill: featured rail order*), which sets `featuredOrder` — the only field the public rail reads — on one document per file in `storage/gallery/featured`. Order is filename-ascending, compared byte-for-byte rather than by locale so the sequence does not depend on the host that ran it. Where a featured file can be identified with an **archive original** the slot is given to the ARCHIVE document instead, so the tile carries that item's caption, operation and credit rather than being a bare photograph; the criterion is exact `bytes` + exact `width`/`height`, all read off fields `index-gallery.mjs` already wrote, so the plan reads no image files (a content hash would mean ~18GB off disk for a one-time backfill). Anything ambiguous keeps its own tile — the four `SelfReason`s are `no-match`, `ambiguous` (two or more archive documents share the fingerprint), `unmeasured` (no bytes/dimensions on the featured document, or a zero, which is a failed sharp probe rather than a photograph) and `archive-claimed` (an earlier featured file already took that document — without which two featured copies of one photograph would write two `featuredOrder` values to one document and leave the rail a tile short). `order` is the index into the sorted list, so the sequence is contiguous from 0 whichever bucket each file fell into.
- **`thumbs.ts`** — `ThumbWidth` (`400 | 800 | 1600`), `THUMB_WIDTHS`, `THUMB_WIDTH` (400), `FEATURED_THUMB_WIDTH` (800), `FEATURED_WIDE_THUMB_WIDTH` (1600), `THUMB_QUALITY` (72), `parseThumbWidth`, `thumbPath(id, width)`, `thumbUrl(id, width)`, `thumbFallbackUrl`. Size/format/path logic for `/api/gallery/media/[id]/thumb`, separated from the route because a `typedRoutes` route file may export nothing but handlers. **The widths are an allow-list and that is a security property**: each one is a file written into the bind-mounted thumb cache and never evicted, so a free `?w=` integer would let an anonymous visitor fill the volume from a single media id. The width is in the CACHE FILENAME and in the ETag — the cache is invalidated against the SOURCE's mtime, so with three live widths on one URL nothing else would distinguish a 400px thumbnail from an 800px one. `thumbUrl` omits `?w=` for the default so there is exactly one URL per size. `thumbFallbackUrl` is where the route redirects when a resize fails: degrade to slow, never to blank.
- **`deps.ts`** — `galleryDeps()`, the one place naming the four collections `RelocateDeps` requires (`galleryMedia`, `operations`, `operationCampaigns`, `campaignMissions`). Deliberately the only module in `lib/gallery` that imports `@/lib/mongo`.
- **`library-query.ts`** — `parseLibraryParams`/`buildLibraryFilter`/`buildLibrarySort`, `PAGE_SIZE` (60). `year`/`campaign`/`operation` each have **two channels**: a string (a literal match) and a `*Unset` boolean (`$exists: false`). They are separate because a real document can hold the literal string `Unknown`, so a sentinel string would make such a document unreachable from the rail.
...
### types/gallery.d.ts
- `ScreenshotOfMonth` — `{filename, mediaId?, dateTaken?, credit, setAt, setBy, operationId?, operationTitle?}`.
- `SotmMediaTileAPI`, `GalleryItemAPI`, `FeaturedItemAPI`, `GalleryAPI` — the public gallery's response shapes.
- `AdminMediaAPI` — one media row as the J5 Media tab receives it (storage key and on-disk filename included, Wilson score omitted). Carries the folder facets `year`/**`campaign`**/`operation`/`opLabel`/`mission`; `campaign` is null for a single mission and every legacy item, and `operation` then names the **campaign mission** rather than the operation. Also carries **`featured`**, a boolean — whether `featuredOrder` is set on the document, never the number itself: the Media tab decides rail membership (the inspector's toggle, the grid's gold star, the table's Featured column) while the Featured tab owns the sequence, and handing this surface a position it cannot meaningfully change is how the two would come to disagree about who owns the order.
- `LibraryOperationRow` — one operation row of the Media tab's tree; named rather than inlined because it renders in two places (under a year, and under one of that year's campaigns).
- `LibraryFacetsAPI` — the Media tab's left rail: saved views plus `years[{year, unset, count, campaigns[{campaign, count, operations[]}], operations[]}]`, tags and authors. A year's campaigns and its campaign-less operations are **two buckets**, not one tree with a synthetic "no campaign" node — most of the archive has no campaign. `unset` on a year/operation row distinguishes "the field is absent" from a document whose field literally is the string `Unknown`; the campaign level needs no such flag because absence is a different bucket there.

### types/gallery-media.d.ts
- `GalleryMedia` — the indexed document behind every gallery tile. `storageKey` prefixes name the directory (`content:`/`legacy:`, `media:`, `featured:`, `sotm:`). The folder facets `year`/`campaign`/`operation`/`opLabel`/`mission`/`operationId` mirror one level of the content path each — see the `lib/gallery/` section above for who writes them.
- `GalleryVote`, `GalleryTag`.
...
- `getGalleryTiles(limit)` → `GalleryTile[]` — a Fisher-Yates pick from `gallery_media` where `featuredOrder` is set, i.e. the same rotation the gallery's featured rail plays, NOT a readdir of `storage/gallery/featured` (nothing prunes that directory when J5 drops an item, so the two sources disagreed the moment `featuredOrder` existed). Each tile carries `src` (the original, rendered by nothing), `thumb` (800px) and `thumbWide` (1600px) from `thumbUrl`, plus a caption falling back to `opLabel`.
