...
- `listBackups(): BackupPoint[]` — merges both repos' `restic snapshots --json` output into hour-bucket points; a bucket can have either or both sides present. Each side carries `dbSizeBytes`/`mediaSizeBytes` from restic's own snapshot `summary.total_bytes_processed`, when the restic version that wrote it recorded one.
