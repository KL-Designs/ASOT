...
- **`naming.ts`** — `splitOperation`, `normalizeKey`, `fullKey`/`strippedKey`, `findByOperationKey`. The **two-tier** folder↔operation match (parenthetical-carrying key first, then stripped). The ORDER is the safety property — read the module comment before touching it.
