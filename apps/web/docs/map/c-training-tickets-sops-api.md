...
#### /api/backups/upload
- **POST** — accepts a multipart-uploaded `.zip` **streamed to disk with busboy**, never buffered: `request.formData()` materialises the whole payload in memory first, and with a 6.5GB gallery an archive containing media simply fails the allocation — surfacing as a bare `Failed to parse form data` that said nothing about size. Bytes go from socket to file as they arrive. The archive lands in `storage/backup-meta/incoming` rather than `os.tmpdir()`, which inside a container is frequently tmpfs (buffering in RAM under another name). An 8GB ceiling is enforced mid-stream (413). The client sends its fields ahead of the file so `parts` is validated on arrival, refusing a bad request in one round trip instead of after gigabytes. Then fire-and-forget `applyUploadedZip(tmpPath, parts, { wipeMedia, wipeDatabase })`; rejects if not idle. Optional `parts` form field (comma-separated `database`/`gallery`/`uploads`, absent = all) restores only those parts of the archive; malformed values are a 400, never a silent widening. Optional `wipeMedia` form field empties each restored media tree first, and optional `wipeDatabase` drops every live collection before reinserting the archive's (both compared `=== 'true'`, so anything else is off); both are recorded in the action log. The dashboard sends both ticked by default. Gate: `backups.restore`. Filesystem only — does not feed the upload into either restic repo's history.

#### /api/backups/revert
- **POST** — reverts to a merged backup point resolved server-side from a client-supplied `id` via `listBackups()` (never trusts the id directly), fire-and-forget `revertToPoint(point, parts, { wipeMedia, wipeDatabase })`; rejects if not idle or point not found. Optional `parts` body array (`database`/`gallery`/`uploads`, absent = all) restores only those; malformed values are a 400 rather than a widened restore, and the chosen parts are recorded in the action log. Optional `wipeMedia` boolean (strict `=== true`) empties each restored media tree before copying, so the result matches the backup instead of merging into what is there; also logged. Defaults to false at the route, but the dashboard ticks it by default — the database half already drops and reinserts, so a clean media restore is the consistent behaviour. Optional `wipeDatabase` boolean (same strict `=== true`, same default, also logged, also ticked by default in the dashboard) drops every collection the live database reports before reinserting the dump's, so a collection the backup predates is destroyed rather than surviving the restore; the library guards it on the dump actually containing collections. A partial revert restores only what was named — the safety backup taken first is always full, so wiped files stay recoverable. Gate: `backups.restore`. Restic restore + filesystem only.
