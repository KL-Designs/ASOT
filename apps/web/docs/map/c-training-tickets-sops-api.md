...
#### /api/backups/upload
- **POST** — accepts a multipart-uploaded `.zip` (buffered fully in memory), writes it to a temp path, then fire-and-forget `applyUploadedZip(tmpPath, parts)`; rejects if not idle. Optional `parts` form field (comma-separated `database`/`gallery`/`uploads`, absent = all) restores only those parts of the archive; malformed values are a 400, never a silent widening. Gate: `backups.restore`. Filesystem only — does not feed the upload into either restic repo's history.

#### /api/backups/revert
- **POST** — reverts to a merged backup point resolved server-side from a client-supplied `id` via `listBackups()` (never trusts the id directly), fire-and-forget `revertToPoint(point, parts)`; rejects if not idle or point not found. Optional `parts` body array (`database`/`gallery`/`uploads`, absent = all) restores only those; malformed values are a 400 rather than a widened restore, and the chosen parts are recorded in the action log. A partial revert restores only what was named — the safety backup taken first is always full. Gate: `backups.restore`. Restic restore + filesystem only.
