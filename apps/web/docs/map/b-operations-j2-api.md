...
- **GET** — bulk field-update endpoint driven entirely by query params (`title`, `date`, `loreDate`, `department`, `themeColor`, `pageTheme`, `coverImage`, `status`, `mapWorld`, `customTheme`, `isSingleMission`, `ownedBy`+`ownedByName`, `billetPoints`). `ownedBy`/`billetPoints` changes require J2 lead or `members.editRestricted`; **`status` requires `operations.overrideLifecycle`** — setting it by hand is an override, not an edit ("In Development" suspends every automation), and normal lifecycle progression no longer comes through here at all (the stage write derives it — see `attendance/platoons` + `statusForStage`). Gate: `PERMISSIONS.operations.write` (plus extra checks for status, owner and billet fields). Collections: `Db.operations`, `Db.operationAttendance` (a `date` change recomputes the derived `rsvpOpenAt` from the stored `rsvpOpenOffsetMins`, so the RSVP window follows the operation). Side effects: `logAction('operation.edit')`.
...
#### /api/operations/[id]/attendance/roles
- **GET** — the roles the board's add-position picker can offer: `_id`, `name`, `tag`, `categories` and nothing else. A deliberately narrow projection: the Roles Manager's own `/api/admin/orbat/roles` returns whole role documents (Discord role ids, TS group ids, granted permission keys) and is gated on `admin.manageOrbat` for that reason — widening *that* gate to everyone who can manage attendance would hand out the unit's grant configuration to make a dropdown work. `categories` is returned rather than filtered server-side because the picker needs it per section and there are five categories on one board. Gate: `attendance.manage` (three-armed). Collections: `Db.orbatRoles`.

#### /api/operations/[id]/attendance/roster
- **POST** — every write to the live attendance board, for both audiences. Member actions (`claim` a specific open position, `leave` it, `prefer` a section/role and sit in the pool) are allowed only while `stage === 'rsvp_open'` — **the RSVP freeze is enforced here, not by a disabled button**. Staff actions (`assign` — which swaps when the destination is occupied, `autofill` the pool honouring preferences, `addSlot` — takes a `roleId`, and **re-checks the role's category scope server-side** via `roleAllowedIn` before writing, because a picker that filters its own dropdown is a convenience and not a rule; `removeSlot`) need `attendance.manage` and are not bound by the window. Every write is computed from `rosterRev` and only lands if that revision is still current, retrying up to 3× against the winner — two members claiming the same position is the normal case on op night, not the edge case. `rosterRev` is also what the live board broadcasts over Y.js. Gate: authenticated for member actions; `attendance.manage` (three-armed, see `platoons`) for staff actions. Collections: `Db.operationAttendance`. Side effects: `logAction('attendance.*')`. Action shapes: `lib/attendance/actions.ts`.
...
- **POST** — member's own RSVP submission (`status: 'attending' | 'not_attending'`, optional `reservistSection`/`reservistRole`); updates existing record or inserts new one keyed off ORBAT position; requires `attendance.rsvpOpen === true`. **No longer writes `orbatRole` on update** — it used to overwrite it with the role taken in another section and overwrite it back on leaving, destroying the only record of where a member belongs; per-operation position is the roster's business now (`roster[].occupantUserId`), so this record keeps ORBAT identity and the roster keeps the assignment. Gate: any authenticated user. Collections: `Db.operationAttendance`, `Db.orbatPositions`.
...
- **POST** — HQ sets `assignedPlatoons[]`, `reservistAssignments[]`, and toggles `rsvpOpen`/`confirmationOpen`/`rsvpOpenOffsetMins`/`rsvpCloseOffsetMins`/`stage` on the attendance doc (upsert); **derives `rsvpOpenAt` from `rsvpOpenOffsetMins` and the op date** — the open end is authored as a lead time, never as an instant; auto-opens RSVP immediately if that derived instant is already in the past and the op isn't `In Development`; mirrors `assignedPlatoons` onto the operation doc; creates section-leader tasks when confirmation newly opens; **derives the operation's `status` from the `stage`** (`op_running` → Active, `confirmations_open` → Completed, via `statusForStage`), which the editor used to do client-side with its own `update?status=` calls; **cuts the attendance roster** via `ensureRosterSnapshot()` the first time the op reaches `rsvp_open` (idempotent — the cron may get there first). Gate: `attendance.manage` (dynamic grant OR its Discord roles OR the legacy `admin.manageOrbat`) — widened from `admin.manageOrbat`, which is J4-only and had been locking out the J2 staff who actually run operations. Collections: `Db.operationAttendance`, `Db.operations`, `Db.orbatPositions`. Side effects: `createAttendanceTasksForOperation()`, `ensureRosterSnapshot()`.
