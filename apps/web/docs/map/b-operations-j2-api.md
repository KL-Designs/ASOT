...
- **POST** — member's own RSVP submission (`status: 'attending' | 'not_attending'`, optional `reservistSection`/`reservistRole`); updates existing record or inserts new one keyed off ORBAT position; requires `attendance.rsvpOpen === true`. **No longer writes `orbatRole` on update** — it used to overwrite it with the role taken in another section and overwrite it back on leaving, destroying the only record of where a member belongs; per-operation position is the roster's business now (`roster[].occupantUserId`), so this record keeps ORBAT identity and the roster keeps the assignment. **Declining here also releases the member from the roster** (`releaseMember`, bumping `rosterRev` so a board write already in flight cannot silently overwrite it) — this route predates the board and only ever wrote the record, which left members stored as the occupant of a position they had declined. Only the release: reclaiming a home position on the way back in displaces somebody and owes them a notification, and both of those live in the board's own roster route. Gate: any authenticated user. Collections: `Db.operationAttendance`, `Db.orbatPositions`.
