...
New collection `department_links`, one flat document per link, following the same flat-collection-plus-order-field pattern used by `board_columns`/`board_cards` (fractional-midpoint reorder) rather than nesting links inside a per-department document, so a single drag-reorder becomes a one-document update, matching every other ordered list in this codebase.
