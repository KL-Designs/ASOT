...
`apps/web/scripts/strip-folder-numbers.ts` (root `npm start` → Migrations → *Strip: gallery folder numbers*, or `npm --prefix apps/web run strip:folder-numbers -- --apply`) is the follow-up to that: folders used to be minted as `{n}. {label}`, the ordering now lives in the database (`takenAt`) and the numbers are re-added only inside a downloaded backup, so this takes the prefix off the folders that already carry one and rewrites the `storageKey` and naming facet of every document under them. Dry-run by default and idempotent; it renames the directory first and updates its documents second, so the only partial state possible is "renamed, documents behind" — which a second run finishes, because it takes its candidates from the storage keys as well as from the disk. It never deletes anything and never merges two folders: every refusal is printed with its reason. The planning half is `lib/gallery/strip-folder-numbers.ts`, which is pure and unit-tested.

`apps/web/scripts/backfill-featured-order.ts` (root `npm start` → Migrations → *Backfill: featured rail order*, or `npm --prefix apps/web run backfill:featured-order -- --apply`) is the repair for the one field `$setOnInsert` could not deliver. The public featured rail reads exactly one thing — `featuredOrder` on `gallery_media` — and no document carried it, so the rail was empty while `storage/gallery/featured` held 58 photographs: `index-gallery.mjs` does compute a `featuredOrder` per featured file, but it merges its facets into `$setOnInsert`, and those documents were inserted by an earlier run that did not carry the field, making every run since a no-op on them. This writes it with `$set`, in filename order, one document per featured file and no other field touched. Where a featured file can be identified with an **archive original** — exact `bytes` plus exact `width`/`height`, read off the document, so nothing is hashed and no image file is opened — the slot goes to the archive document instead, so the tile carries its caption, operation and credit; anything with no match, two matches, no measurements, or a match another featured file already took keeps its own tile, and the dry run prints every file with the bucket it fell into. It refuses to run once **any** document carries `featuredOrder`, so it can never renumber a rotation J5 has curated in the console's Featured tab — re-ordering is `PUT /api/gallery/admin/featured/order` and nothing else. The planning half is `lib/gallery/featured-order.ts`, which is pure and unit-tested. The database half is `lib/gallery/featured-order-backfill.ts` — the guard, the two queries and the `$set` bulkWrite — because there are **two** ways to run this and they must not be able to disagree about when it is safe to write: the script above, and the **J4 console → Tools → Featured Rail Order** card, which posts to `POST /api/admin/gallery/featured-order-backfill` (J4-Administration only, audited as `gallery.featured.backfill` and only when it wrote). The card's dialog keeps the CLI's flow rather than collapsing to one click — it runs the dry pass on open, renders both buckets, and only then offers Apply — because the archive matching is a judgement for the operator and the write happens exactly once. The script and the route are both terminal: they render, and neither holds any of the decision.

Gallery images are served resized, never at full resolution, everywhere but the lightbox. `/api/gallery/media/[id]/thumb` resizes once to WebP and caches under `storage/gallery/thumbs/{id}-{width}.webp`; the width comes from `?w=` and is an **allow-list of three** in `lib/gallery/thumbs.ts` — 400 (J5 Media grid), 800 (public featured rail, the J5 Featured tab's rotation tiles, the home-page mosaic's single-column tiles), 1600 (the home-page mosaic's double-width tiles). Not a free integer: every distinct width is a file written into the bind-mounted thumb cache that nothing evicts, so an arbitrary `?w=` would be a resize-on-demand endpoint any visitor could loop over to fill the volume. `parseThumbWidth` folds anything unrecognised back to 400 rather than erroring. The width lives in the cache filename **and** in the ETag, because the cache is invalidated against the source's mtime and with three widths on one URL nothing else would tell a 400px thumbnail from an 800px one; there is no `immutable`, and a failed resize 307s to the original. The **lightbox keeps the original** (`FeaturedItemAPI.src`) — someone who opens a photograph is asking to look at it.
