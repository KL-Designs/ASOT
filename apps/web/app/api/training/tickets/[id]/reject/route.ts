...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    ...
    if (ticket.status === 'approved' || ticket.status === 'rejected') {
        return NextResponse.json({ error: 'Ticket is already finalised' }, { status: 400 })
    }
    ...
    createNotification({
        ...
        title: 'Training Ticket Rejected',
        body: `Your training ticket for "${ticket.trainingTypeName}" was not approved${j3Notes ? `: ${j3Notes}` : '.'}`,
        actionUrl: '/dashboard/unit/training-docs',
        relatedId: id,
    }).catch(console.error)

    logAction({
        action: 'training.ticket.reject',
        category: 'training',
        performedBy: me.id,
        ...
        details: j3Notes ? { reason: j3Notes } : undefined,
    }).catch(console.error)
