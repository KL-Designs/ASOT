...
import {
    TextField, Typography, Box, InputAdornment, CircularProgress, Autocomplete,
} from '@mui/material'
...
interface DepartmentRoleChip { id: string; department: string; name: string }
interface PermissionNode { key: string; discordRoles: DiscordRoleChip[]; orbatRoles: OrbatRoleChip[]; departmentRoles: DepartmentRoleChip[]; memberCount: number }
interface PermissionCategory { key: string; label: string; permissions: PermissionNode[] }
interface PermissionGrant { granted: boolean; viaDiscordRoles: string[]; viaOrbatRoles: string[]; viaDepartmentRoles: string[]; viaGlobalOverride: boolean }
...
function matchesSearch(node: PermissionNode, term: string): boolean {
    ...
    if (node.departmentRoles.some(r => r.name.toLowerCase().includes(t) || r.department.toLowerCase().includes(t))) return true
    return false
}
...
function DeptRoleChip({ chip, matched }: { chip: DepartmentRoleChip; matched?: boolean }) {
    return (
        <span style={{
            display: 'inline-flex', alignItems: 'center', fontSize: '0.66rem', padding: '2px 8px', borderRadius: 999,
            background: matched ? 'rgba(0,195,100,0.12)' : 'rgba(0,150,255,0.08)',
            border: `1px solid ${matched ? 'rgb(0,195,100)' : 'rgba(0,150,255,0.35)'}`,
            color: matched ? 'rgb(0,195,100)' : 'rgba(140,200,255,0.85)',
        }}>
            {chip.department.toUpperCase()}: {chip.name}
        </span>
    )
}
        ...
        <div style={{
            ...
            <div style={{ minWidth: 0, flex: 1 }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5, alignItems: 'center' }}>
                    ...
                    {node.departmentRoles.map(r => (
                        <DeptRoleChip key={r.id} chip={r} matched={grant?.viaDepartmentRoles.includes(r.id) ?? false} />
                    ))}
                    {node.discordRoles.length === 0 && node.orbatRoles.length === 0 && node.departmentRoles.length === 0 && (
...
export default function PermissionsExplorerTab() {
    ...
    return (
        <Box sx={{ p: 2, display: 'flex', flexDirection: 'column', gap: 1.5, overflow: 'hidden', flex: 1, minHeight: 0 }}>
