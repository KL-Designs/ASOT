...
// Selectable roles per department for task assignment
const DEPT_TASK_ROLES: Record<MeetingDepartment, string[]> = {
    j1: ['J1-Staff', 'J1-Recruiting', 'J4-Administration'],
    j2: ['J2-Team Lead', 'J2-Mission Making', 'J4-Administration'],
    j3: ['J3-Team Lead', 'J3-Training', 'J4-Administration'],
    j4: ['J4-Administration'],
    j5: ['J5-Media', 'J4-Administration'],
    j6: ['J6-Department Lead', 'J6-Game Master', 'J4-Administration'],
    j7: ['J7 Staff', 'J7 Community Development', 'J4-Administration'],
}
...
export default function MeetingTaskList({ meetingId, tasks, locked, userId: _userId, department, meeting: _meeting, onUpdate }: Props) {
    ...
    const availableRoles = DEPT_TASK_ROLES[department] ?? []
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            ...
            {pending.map(task => (
                <TaskRow key={task.id} task={task} locked={locked} isExpanded={expanded.has(task.id)} isUpdating={updating === task.id}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8, padding: '10px 12px', border: '1px solid rgba(219,0,29,0.25)', background: 'rgba(219,0,29,0.03)', marginTop: 6 }}>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                        ...
                        <div style={{ gridColumn: '1 / -1' }}>
                            ...
                            <input autoFocus value={title} onChange={e => setTitle(e.target.value)} onKeyDown={e => { if (e.key === 'Escape') resetForm() }} placeholder='What needs to be done?' style={inputSx} />
                        </div>
            ...
            <ConfirmDialog
                ...
                title='Delete task?'
                message={`"${tasks.find(t => t.id === confirmDeleteId)?.title ?? 'This task'}" will be permanently deleted.`}
                danger
                confirmLabel='Delete'
                onConfirm={() => { const tid = confirmDeleteId!; setConfirmDeleteId(null); deleteTask(tid) }}
...
function TaskRow({ task, locked, isExpanded, isUpdating, onToggleExpand, onCycleStatus, onDelete }: {
    task: MeetingTask; locked: boolean; isExpanded: boolean; isUpdating: boolean
    onToggleExpand: () => void; onCycleStatus: () => void; onDelete: () => void
}) {
    return (
        <div style={{ border: '1px solid rgba(255,255,255,0.05)', background: 'rgba(255,255,255,0.02)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 8px' }}>
                <button onClick={onCycleStatus} disabled={locked || isUpdating}
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                        ...
                        {task.carriedOverFrom && (
                            <span style={{ fontSize: '0.54rem', padding: '1px 4px', background: 'rgba(0,195,255,0.08)', border: '1px solid rgba(0,195,255,0.2)', color: 'rgba(0,195,255,0.6)', fontWeight: 700, letterSpacing: '0.06em' }}>CARRIED OVER</span>
                        )}
                    </div>
                    <div style={{ display: 'flex', gap: 8, marginTop: 2, flexWrap: 'wrap' }}>
                        {task.assignedToName && <span style={{ display: 'flex', alignItems: 'center', gap: 3, fontSize: '0.58rem', color: 'rgba(219,0,29,0.6)' }}><Person sx={{ fontSize: 9 }} />{task.assignedToName}</span>}
                        {task.assignedRole && !task.assignedToName && <span style={{ fontSize: '0.58rem', color: 'rgba(219,0,29,0.5)' }}>@{task.assignedRole}</span>}
                        {task.dueDate && <span style={{ display: 'flex', alignItems: 'center', gap: 3, fontSize: '0.58rem', color: 'rgba(237,237,237,0.3)' }}><CalendarToday sx={{ fontSize: 9 }} />Due {fmtDate(task.dueDate)}</span>}
                        {task.chaseUpDate && <span style={{ display: 'flex', alignItems: 'center', gap: 3, fontSize: '0.58rem', color: 'rgba(255,160,0,0.5)' }}><NotificationsActive sx={{ fontSize: 9 }} />Chase {fmtDate(task.chaseUpDate)}</span>}
                    </div>
                </div>
                <div style={{ display: 'flex', gap: 3, flexShrink: 0 }}>
                    ...
                    {!locked && <button onClick={onDelete} disabled={isUpdating} style={{ all: 'unset', cursor: 'pointer', color: 'rgba(219,0,29,0.3)', display: 'flex' }}><Delete sx={{ fontSize: 13 }} /></button>}
                </div>
            </div>
            {isExpanded && (
                <div style={{ padding: '0 8px 8px 30px', display: 'flex', flexDirection: 'column', gap: 4, borderTop: '1px solid rgba(255,255,255,0.04)' }}>
                    {task.description && <p style={{ margin: 0, fontSize: '0.72rem', color: 'rgba(237,237,237,0.5)', lineHeight: 1.55, paddingTop: 6 }}>{task.description}</p>}
                    <div style={{ display: 'flex', gap: 16, marginTop: 4, flexWrap: 'wrap' }}>
                        {task.dueDate && <div style={{ fontSize: '0.62rem', color: 'rgba(237,237,237,0.4)' }}><span style={{ color: 'rgba(237,237,237,0.25)', marginRight: 4 }}>DUE</span>{new Date(task.dueDate).toLocaleString('en-AU', { dateStyle: 'medium', timeStyle: 'short' })}</div>}
                        {task.chaseUpDate && <div style={{ fontSize: '0.62rem', color: 'rgba(255,160,0,0.55)' }}><span style={{ color: 'rgba(255,160,0,0.3)', marginRight: 4 }}>CHASE-UP</span>{new Date(task.chaseUpDate).toLocaleString('en-AU', { dateStyle: 'medium', timeStyle: 'short' })}</div>}
                        {task.completedByName && <div style={{ fontSize: '0.62rem', color: 'rgba(0,200,80,0.5)' }}>Completed by {task.completedByName}</div>}
                    </div>
                </div>
            )}
        </div>
