...
export default function PerformanceReportTab({ userId }: { userId: string }) {
    ...
    const fetchTickets = useCallback(async () => {
        ...
        try {
            ...
            setMyTickets((data.tickets ?? []).filter((t: TicketRow) => t.type === 'performance-report'))
        } finally {
    ...
    async function handleSubmit() {
        ...
        try {
            ...
            const data = await res.json()
            if (!res.ok) {
                setSubmitError(data.error || 'Failed to submit performance report')
                return
            }
            ...
            setReason('')
            setNotes('')
            setSubmitSuccess('Performance report submitted. J4 will acknowledge the report.')
            fetchTickets()
        } finally {
            setSubmitting(false)
    ...
    return (
        <div className='p-6 flex flex-col gap-5'>
            {/* Submit Form */}
            <div style={cardStyle}>
                <Typography style={labelStyle}>Submit Performance Report</Typography>
                <div className='flex flex-col gap-4'>
                    ...
                    <div>
                        ...
                        >
                            {submitting ? 'Submitting…' : 'Submit Performance Report'}
                        </Button>
                    </div>
                </div>
            </div>
