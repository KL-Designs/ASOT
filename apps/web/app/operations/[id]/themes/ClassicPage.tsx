...
import type { ThemePageProps } from './theme-props'
...
/**
 * The operation orders page as it has always looked — now serving the
 * `oldfashioned` and `scifi` themes only.
 *
 * Lifted out of `page.tsx` unchanged when Modern was rebuilt (see
 * `ModernPage.tsx`), so those two themes render byte-for-byte what they did
 * before. That is the whole point of the split: one theme was being redesigned
 * and the other two were not, and a shared component full of three-way
 * ternaries meant every change to one risked the others.
 *
 * Its `isModern` branches are therefore dead — the dispatcher in `page.tsx`
 * never sends Modern here. They are left in place rather than pruned, because
 * pruning them by hand through this many nested ternaries is exactly the kind
 * of edit that silently breaks the two themes nobody asked to change. The next
 * person to touch this should split it into `OldFashionedPage` and `SciFiPage`
