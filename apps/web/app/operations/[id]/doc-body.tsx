 ...
 *
 * Note what is *not* here: the `coldwar` palette. Rules injected from this file
 * land at `.op-doc` — the same specificity as the base rule in globals.css — so
 * they only win on source order, and they do not. That is invisible while every
 * theme is dark (losing the tie leaves you with near-white text on a dark
 * ground, which is roughly what you wanted anyway) and glaring the moment a
 * theme is light: body text stayed pale on paper while `strong`, whose selector
 * is one step more specific, went black. `oldfashioned` has always worked around
 * it by restating the colour on `p` and `li`.
 *
 * Themes that need to be sure should override from their own module with
 * `.wrapper :global(.op-doc)`, which outranks the base rule outright — see
 * `themes/modern.module.css` and `themes/coldwar.module.css`.
 *
